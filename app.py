...
def similarity(text1, text2):
    ...
    text1_concat = ' '.join(text1)
    text2_concat = ' '.join(text2)
    tfidf_matrix = tfidf_vectorizer.fit_transform([text1_concat, text2_concat])
    return cosine_similarity(tfidf_matrix)[0][1]

# Define search function
def search_products(query):
    stm_query = tokenize_and_stem(query)
    data['similarity'] = data['stemmed_tokens'].apply(lambda x: similarity(stm_query, x))
